...
- Use Laravel notification channel `sms` as the default way to send user-facing SMS notifications. Existing app notifications can append `sms` to `via()` and define `toSms()`.
- Build SMS payloads with `Codepreneur\SentDm\Messages\SentDmSmsMessage`; use `text()` for default-template SMS and `forTemplate()` or `templateArray()` for explicit Sent DM templates.
- Use `Codepreneur\SentDm\Notifications\SmsNotification` for one-off text SMS sends.
- Notifiable models should expose `routeNotificationForSms()` unless a `SentDmSmsMessage` explicitly sets recipients with `to()`.
- Use `Codepreneur\SentDm\Concerns\InteractsWithSentDmSms` when notifications should include `sms` only if an SMS route exists and should use the configured SMS queue/backoff behavior.
- Configure credentials with `SENT_DM_API_KEY`; never hard-code API keys or webhook secrets.
- Configure simple text SMS with `SENT_DM_SMS_TEMPLATE_NAME`, `SENT_DM_SMS_TEMPLATE_PARAMETER`, and optionally `SENT_DM_SMS_TEMPLATE_ID`.
